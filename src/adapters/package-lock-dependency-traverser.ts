...
import {
  ...
  type NormalizedLockfileProject,
  traverseNormalizedLockfileProject,
...
export class PackageLockDependencyTraverser implements PackageLockDependencyTraverserPort {
  ...
  async traverse(package_lock_path: string, max_depth: number): Promise<TraversedDependencyGraph> {
    ...
    const project = createNormalizedPackageLockProject(lockfile, package_lock_path)
...
function createNormalizedPackageLockProject(
  lockfile: PackageLockFile,
  packageLockPath: string,
  ...
  return {
    root_package: resolveRootPackage(lockfile, packageLockPath),
    root_dependencies: Object.fromEntries(
      resolveRootDependencies(lockfile).map((dependencyName) => [dependencyName, '*']),
    ),
    resolve_root_dependency(dependencyName: string): NormalizedLockfileEntry | null {
      ...
      return entryPath === null ? null : packageEntries.get(entryPath) ?? null
    },
    resolve_entry_dependency(entry: NormalizedLockfileEntry, dependencyName: string): NormalizedLockfileEntry | null {
      const entryPath = resolveDependencyEntryPath(entry.entry_id, dependencyName, packageEntries)
